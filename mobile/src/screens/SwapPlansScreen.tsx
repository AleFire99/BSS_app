...
export default function SwapPlansScreen({ route, navigation }: Props) {
  ...
  const handleCollapseEnd = async (planId: number) => {
    ...
    catch (e: any) { Alert.alert(t('common.error'), e.message); load(); }
  };
  ...
  return (
    <View style={styles.container}>
      <FlatList
              ...
              >
                <View style={styles.planInfo}>
                  ...
                  {item.cards.length > 0 ? (
                    ...
                    <Text style={styles.planSub}>{t('swapPlans.emptyPlan')}{item.notes ? `  ·  ${item.notes}` : ''}</Text>
                  )}
                </View>
        ...
        ListEmptyComponent={
          <Text style={styles.empty}>{t('swapPlans.emptyPlans')}</Text>
        }
      ...
      {copying && (
        <View style={styles.copyingBanner}>
          ...
          <Text style={styles.copyingText}>{t('swapPlans.copyingPlan')}</Text>
        </View>
      )}
        ...
        <View style={styles.sheetWrap} pointerEvents="box-none">
          <View style={styles.contextSheet}>
            ...
            <TouchableOpacity style={styles.menuRow} onPress={() => contextPlan && openRename(contextPlan)}>
              ...
              <Text style={styles.menuLabel}>{t('swapPlans.rename')}</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.menuRow} onPress={() => contextPlan && handleCopy(contextPlan)}>
              ...
              <Text style={styles.menuLabel}>{t('swapPlans.copy')}</Text>
            </TouchableOpacity>
            ...
            <TouchableOpacity style={styles.menuRow} onPress={() => contextPlan && handleContextDelete(contextPlan)}>
              ...
              <Text style={[styles.menuLabel, styles.menuDanger]}>{t('swapPlans.delete')}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
      ...
      <Modal visible={!!confirmDeletePlan} transparent animationType="fade">
        ...
        <View style={styles.dialogWrap} pointerEvents="box-none">
          <View style={styles.dialog}>
            <Text style={styles.dialogTitle}>{t('swapPlans.deletePlan')}</Text>
            <Text style={styles.dialogBody}>{t('swapPlans.deleteBody', { name: confirmDeletePlan?.name })}</Text>
            <View style={styles.dialogActions}>
              <TouchableOpacity style={styles.dialogBtn} onPress={() => setConfirmDeletePlan(null)}>
                <Text style={[styles.dialogBtnText, { color: theme.textMuted }]}>{t('swapPlans.cancel')}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.dialogBtn} onPress={handleConfirmDelete}>
                <Text style={[styles.dialogBtnText, { color: '#ef5350' }]}>{t('swapPlans.delete')}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      ...
      <Modal visible={createModal} transparent animationType="fade">
        ...
        >
          <View style={styles.sheet}>
            <Text style={styles.sheetTitle}>{t('swapPlans.newPlan')}</Text>
            <TextInput
              style={styles.input}
              placeholder={t('swapPlans.planNamePlaceholder')}
            ...
            <TouchableOpacity style={styles.btn} onPress={handleCreate} disabled={!planName.trim()}>
              <Text style={styles.btnText}>{t('swapPlans.create')}</Text>
            </TouchableOpacity>
          </View>
        </KeyboardAvoidingView>
      </Modal>
      ...
      <Modal visible={renameModal} transparent animationType="fade">
        ...
        >
          <View style={styles.sheet}>
            <Text style={styles.sheetTitle}>{t('swapPlans.renamePlan')}</Text>
            <TextInput
              style={styles.input}
              value={renameName}
            ...
            <TouchableOpacity style={styles.btn} onPress={handleRename} disabled={!renameName.trim()}>
              <Text style={styles.btnText}>{t('swapPlans.save')}</Text>
            </TouchableOpacity>
          </View>
        </KeyboardAvoidingView>
      </Modal>
